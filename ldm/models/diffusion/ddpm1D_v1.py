...
class DDPM(pl.LightningModule):
    ...
    def _get_rows_from_list(self, samples):
        ...
        denoise_grid = rearrange(samples, 'n b c h -> b n c h')
        denoise_grid = rearrange(denoise_grid, 'b n c h -> (b n) c h')
...
class LatentDiffusion(DDPM):
    ...
    def _get_denoise_row_from_list(self, samples, desc='', force_no_decoder_quantization=False):
        ...
        denoise_grid = rearrange(denoise_row, 'n b c h -> b n c h')
        denoise_grid = rearrange(denoise_grid, 'b n c h -> (b n) c h')
        denoise_grid = make_grid(denoise_grid, nrow=n_imgs_per_row)
        return denoise_grid
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        if isinstance(encoder_posterior, (DiagonalGaussianDistribution,DiagonalGaussianDistribution1D)):
            ...
            # autoencoder1D_v1.py:100 
            # def encode(self, x): 
            # return quant, emb_loss, info
           z = encoder_posterior[0]
        else:
            # print("###",len(encoder_posterior),"###",type(encoder_posterior[0]),encoder_posterior[0].shape)
    ...
    def delta_border(self, h):
        ...
        """
        lower_right_corner = torch.tensor([h - 1]).view(1, 1, 1)
        arr = self.meshgrid(h) / lower_right_corner
        dist_left_up = torch.min(arr, dim=-1, keepdims=True)[0]
        dist_right_down = torch.min(1 - arr, dim=-1, keepdims=True)[0]
        edge_dist = torch.min(torch.cat([dist_left_up, dist_right_down], dim=-1), dim=-1)[0]
        return edge_dist

    def get_weighting(self, h, Ly, device):
        weighting = self.delta_border(h)
        weighting = torch.clip(weighting, self.split_input_params["clip_min_weight"],
                               self.split_input_params["clip_max_weight"], )
        weighting = weighting.view(1, h , 1).repeat(1, 1, Ly).to(device)

        if self.split_input_params["tie_braker"]:
            L_weighting = self.delta_border(Ly)
            L_weighting = torch.clip(L_weighting,
                                     self.split_input_params["clip_min_tie_weight"],
                                     self.split_input_params["clip_max_tie_weight"])

            L_weighting = L_weighting.view(1, 1, Ly).to(device)
    ...
    def get_fold_unfold(self, x, kernel_size, stride, uf=1, df=1):  # todo load once not every time, shorten code
        ...
        if uf == 1 and df == 1:
            ...
            weighting = self.get_weighting(kernel_size[0], Ly, x.device).to(x.dtype)
            normalization = fold(weighting).view(1, 1, h)  # normalizes the overlap
            weighting = weighting.view((1, 1, kernel_size[0], Ly))
            ...
            fold = torch.nn.Fold(output_size=(x.shape[2] * uf), **fold_params2)

            weighting = self.get_weighting(kernel_size[0] * uf, Ly, x.device).to(x.dtype)
            normalization = fold(weighting).view(1, 1, h * uf)  # normalizes the overlap
            weighting = weighting.view((1, 1, kernel_size[0] * uf,Ly))
            ...
            fold = torch.nn.Fold(output_size=(x.shape[2] // df), **fold_params2)

            weighting = self.get_weighting(kernel_size[0] // df, Ly, x.device).to(x.dtype)
            normalization = fold(weighting).view(1, 1, h // df)  # normalizes the overlap
            weighting = weighting.view((1, 1, kernel_size[0] // df, Ly))
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                z = z.view((z.shape[0], -1, ks[0], z.shape[-1]))  # (bn, nc, ks[0], ks[1], L )
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        # https://github.com/CompVis/latent-diffusion/issues/228
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if plot_diffusion_rows:
            ...
            diffusion_grid = rearrange(diffusion_row, 'n b c h -> b n c h')
            diffusion_grid = rearrange(diffusion_grid, 'b n c h -> (b n) c h')
        ...
        if sample:
            ...
            if inpaint:
                ...
                b, h, = z.shape[0], z.shape[2]
                mask = torch.ones(N, h,).to(self.device)
