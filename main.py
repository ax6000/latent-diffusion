...
class NpyLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
                  global_step, current_epoch, batch_idx):
        root = os.path.join(save_dir, "npy", split)
        # for k in images:
        #     fig = self.make_figures(images[k])
        for k in images.keys():
            shape_str = "x".join([str(x) for x in images[k].shape])
            path = os.path.join(root, f"{k}-{batch_idx}-{shape_str}-samples.npy")
            np.save(path,images[k])
